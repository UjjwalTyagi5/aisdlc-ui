# backend/tests/test_development_agent_chat_access.py
import json
...
async def project_with_org_admin():
    # AGENT_DEFAULT_REACH["development"] (backend/config/agent_registry.py) has no
    # "org_admin" key at all (Organization Admin holds zero agent access by design --
    # spec §1.4), so org_admin is the genuine default-deny caller for this route, same
    # pattern test_security_agent_chat_access.py established.
    org = str(_uuid.uuid4())
    ...
    async with get_db_session_superuser() as s:
        await s.execute(text(
            "INSERT INTO organizations (id, slug, display_name) VALUES (:i, :s, 'Dev Chat Test')"
        ), {"i": org, "s": f"dev-chat-{org[:8]}"})
        await s.execute(text(
...
def _pipeline_ctx(project_id: str) -> str:
    return json.dumps({"project_id": project_id})


def test_an_org_admin_without_development_access_is_refused_before_any_work_runs(project_with_org_admin):
    t = project_with_org_admin
    resp = _client().post(
        "/sdlc/agent/development/chat/",
        data={
            "session_id": str(_uuid.uuid4()),
            "user_id": t["org_admin"],
            "task_intent": "write some code",
            "pipeline_context": _pipeline_ctx(t["project"]),
        },
...
async def test_the_form_user_id_can_no_longer_impersonate_someone_else(project_with_org_admin):
    """The real, authenticated caller is the org_admin (no default development access),
    ...
    Form field, this would succeed; since identity comes from the verified session, it
    must still 403.
    """
    t = project_with_org_admin
    ...
    async with get_db_session_superuser() as s:
        await s.execute(text(
            "INSERT INTO users (id, tenant_id, email) VALUES (:i, :t, 'overridden-dev@example.com')"
    ...
    async with get_db_session_for_tenant(t["org"]) as s:
        await s.execute(text(
            ...
            "VALUES (:i, :t, :p, :u, 'development', 'use')"
...
async def test_a_role_held_only_on_a_different_project_does_not_reach_this_ones_development_agent():
    """A Developer added to Project A must NOT be able to target Project B's
    Development agent and be granted "build" access just because
    AGENT_DEFAULT_REACH["development"]["developer"] == "build" -- they hold no binding
    on Project B at all. Same cross-project-leakage class as Security's own regression
    test; assert_agent_access_for_chat's membership check exists for exactly this.
    """
    org = str(_uuid.uuid4())
    unit = str(_uuid.uuid4())
    ...
    async with get_db_session_superuser() as s:
        await s.execute(text(
            "INSERT INTO organizations (id, slug, display_name) VALUES (:i, :s, 'Dev Membership Test')"
        ), {"i": org, "s": f"dev-membership-{org[:8]}"})
        await s.execute(text(
            "INSERT INTO workspaces (id, organization_id, slug, display_name) "
            "VALUES (:i, :o, 'unit', 'Unit')"
        ), {"i": unit, "o": org})
    ...
    resp = _client().post(
        ...
        data={
            ...
            "user_id": dev,
            "task_intent": "write some code",
            "pipeline_context": _pipeline_ctx(project_b),
        },
