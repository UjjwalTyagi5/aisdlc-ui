...
# `publish_to_sharepoint` USED TO LIVE HERE and has been deleted, not merely
# unbound. It filed `session.generated_docs` -- documents written this
# conversation and reviewed by nobody -- into the business's SharePoint library.
# The agent now uses `shared/tools/sharepoint_artifacts.publish_approved_to_sharepoint`
# like every other agent, which publishes approved artifacts only. Leaving the old
# one importable would let it be rebound by accident and quietly reopen the hole.
...
async def publish_to_confluence(space: str = "", filename: str = "", parent_id: str = "") -> str:
    ...
    # APPROVED ONLY, the same gate SharePoint publishing uses. A Confluence space is
    # the business's wiki: what lands there is read by people who were not in this
    # conversation and cannot tell a draft from a signed-off document. This used to
    # publish straight from `s.generated_docs` — written minutes earlier, reviewed by
    # nobody — which made "approved" mean nothing on the one destination most likely to
    # be read by somebody outside the project.
    #
    # `save_document` now records every deliverable as a PENDING artifact, so there is
    # an approval to wait on. Matching is by FILENAME because that is the only field
    # the session doc and the artifact row share.
    approved_names: set = set()
    try:
        from shared.tools.sharepoint_artifacts import _approved_documents  # noqa: PLC0415
        from config.ws_helper import get_project_id, get_tenant_id  # noqa: PLC0415

        approved_names = {
            d["name"] for d in await _approved_documents(
                get_tenant_id() or getattr(s, "tenant_id", "") or "",
                get_project_id() or getattr(s, "project_id", "") or "",
                "documentation",
            )
        }
    except Exception:  # noqa: BLE001
        # FAIL CLOSED. If the approved set cannot be read, nothing is published --
        # the alternative is publishing unreviewed documents to the company wiki
        # because a database call failed, which is the outcome this gate exists to
        # prevent.
        logger.warning("publish_to_confluence: could not resolve approved documents",
                       exc_info=True)
    ...
    if not docs:
        ...
        return ("ERROR: none of these documents are approved yet, so nothing was "
                f"published to Confluence: {', '.join(blocked) or '(none generated)'}. "
                "An owner approves them in the Documents panel on this screen.")
...
@tool
async def save_handover_document(
    title: str, filename: str, scope: str, systems_and_access: str,
    operational_runbook: str, in_flight_work: str, known_risks: str,
    key_contacts: str, first_90_days: str = "",
) -> str:
    """Save a HANDOVER document — the pack a team receiving ownership of this system
    needs on day one. Use when work is being transferred to another team, a vendor, or
    a support function.

    A HANDOVER IS NOT A KT DOCUMENT. This one answers "what am I now accountable for,
    and what is on fire" — it is written for a team taking over responsibility. Use
    save_kt_document instead when the reader is an individual joining the existing team.

    Fill every argument from what you actually read in the repo, the upstream artifacts
    and the connected wiki. Where something genuinely is not known, write "Not
    documented — <who to ask>" rather than inventing it: a handover that quietly omits
    a gap is worse than one that names it, because the receiving team discovers it in
    production instead.

    Args:
        title: e.g. "Handover — Payments Service".
        filename: kebab-case, ending in .md.
        scope: what is and is NOT being handed over. Name the exclusions explicitly.
        systems_and_access: repos, environments, dashboards, secrets stores, and the
                            access each one needs plus who grants it.
        operational_runbook: how it is deployed, monitored and rolled back; the alerts
                             that fire and what to do about each.
        in_flight_work: branches, open PRs, partially delivered work, and its state.
        known_risks: defects, tech debt, expiring credentials, single points of
                     knowledge. Include severity and any deadline.
        key_contacts: who to ask about what, and the escalation path.
        first_90_days: optional recommended actions for the receiving team.
    """
    missing = [n for n, v in (
        ("scope", scope), ("systems_and_access", systems_and_access),
        ("operational_runbook", operational_runbook), ("in_flight_work", in_flight_work),
        ("known_risks", known_risks), ("key_contacts", key_contacts),
    ) if not (v or "").strip()]
    if missing:
        # REFUSED RATHER THAN SAVED WITH HOLES. A handover is read once, under time
        # pressure, by somebody who cannot tell an omitted section from one that did
        # not apply.
        return (f"ERROR: a handover document cannot be saved without {', '.join(missing)}. "
                "Read the repo, the upstream artifacts and the wiki first; if something "
                "genuinely is not documented, say so in the section and name who to ask.")
    body = (
        f"# {title or 'Handover Document'}\n\n"
        f"## Scope of this handover\n{scope.strip()}\n\n"
        f"## Systems and access\n{systems_and_access.strip()}\n\n"
        f"## Operating the system\n{operational_runbook.strip()}\n\n"
        f"## Work in flight\n{in_flight_work.strip()}\n\n"
        f"## Known risks and open issues\n{known_risks.strip()}\n\n"
        f"## Key contacts and escalation\n{key_contacts.strip()}\n"
    )
    if (first_90_days or "").strip():
        body += f"\n## Recommended first 90 days\n{first_90_days.strip()}\n"
    return await save_document.ainvoke({
        "doc_type": "handover", "title": title, "filename": filename,
        "markdown_contents": body,
    })


@tool
async def save_kt_document(
    title: str, filename: str, what_it_does: str, architecture_tour: str,
    environments_and_setup: str, common_tasks: str, where_things_live: str,
    glossary: str = "", first_week_checklist: str = "",
) -> str:
    """Save a KT (knowledge transfer) document — what somebody joining this project
    needs in order to become useful. Written for a person, not a team.

    Use when the user asks to onboard a new joiner, write KT material, or explain the
    project to somebody new. For transferring OWNERSHIP of a system to another team,
    use save_handover_document instead.

    WRITE IT FOR SOMEBODY WITH NO CONTEXT. The failure mode of KT material is assumed
    knowledge — an acronym nobody expands, a "just run the usual script". Expand every
    project-specific term the first time it appears, and prefer the exact command over
    a description of it.

    Args:
        title: e.g. "Knowledge Transfer — Payments Service".
        filename: kebab-case, ending in .md.
        what_it_does: the system's purpose in plain language, and who depends on it.
        architecture_tour: the main components and how a request flows through them.
                           Reference real paths in the repo.
        environments_and_setup: which environments exist, and the exact steps to run it
                                locally, including prerequisites.
        common_tasks: the handful of things this person will actually be asked to do,
                      each with its steps.
        where_things_live: repos, docs, dashboards, boards, channels.
        glossary: optional project-specific terms and acronyms, expanded.
        first_week_checklist: optional ordered list for their first week.
    """
    missing = [n for n, v in (
        ("what_it_does", what_it_does), ("architecture_tour", architecture_tour),
        ("environments_and_setup", environments_and_setup),
        ("common_tasks", common_tasks), ("where_things_live", where_things_live),
    ) if not (v or "").strip()]
    if missing:
        return (f"ERROR: a KT document cannot be saved without {', '.join(missing)}. "
                "Inspect the repo and read the upstream artifacts first — KT material "
                "written from assumption is what makes a new joiner distrust the docs.")
    body = (
        f"# {title or 'Knowledge Transfer'}\n\n"
        f"## What this system does\n{what_it_does.strip()}\n\n"
        f"## Architecture tour\n{architecture_tour.strip()}\n\n"
        f"## Environments and local setup\n{environments_and_setup.strip()}\n\n"
        f"## Common tasks\n{common_tasks.strip()}\n\n"
        f"## Where things live\n{where_things_live.strip()}\n"
    )
    if (glossary or "").strip():
        body += f"\n## Glossary\n{glossary.strip()}\n"
    if (first_week_checklist or "").strip():
        body += f"\n## First week\n{first_week_checklist.strip()}\n"
    return await save_document.ainvoke({
        "doc_type": "kt", "title": title, "filename": filename,
        "markdown_contents": body,
    })
