...
logger.info("Configuring LLM via LiteLLM proxy...")
load_dotenv()
# genai still used for embeddings (genai.embed_content) — keep configured for that purpose
...
def rca_agent(state: AgentState) -> AgentState:
    ...
    data = clean_json_response(response.choices[0].message.content or "")
