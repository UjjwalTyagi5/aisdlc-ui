...
def _paint_runs(canvas: st.WordCanvas, p, runs: list[tuple[str, dict]], *, size: float = 10,
    ...
    for text, kw in runs:
        kw = dict(kw)
        canvas.run(p, text, size=kw.pop("size", size), colour=kw.pop("colour", colour), **kw)
...
def pill_for(value: str, *, column: str = "") -> Optional[tuple[str, str]]:
    ...
    if key not in _PILLS:
        return None
    if _SEVERITY_COLUMN_RE.search(column or "") or key in ("accepted", "proposed", "deprecated", "superseded", "rejected"):
        return _PILLS[key]
    return None
...
class _Painter:
    def __init__(self, canvas: st.WordCanvas, render_mermaid: Optional[RenderMermaid],
                 fetch_image: Optional[FetchImage]):
        self.c = canvas
    ...
    def heading(self, level: int, text: str) -> None:
        ...
        if level == 2:
            self.current_section = _section_label(text)
    ...
    def table(self, rows: list[list[str]]) -> None:
        ...
        if cols >= 3:
            widths[0] = st.PAGE_WIDTH_MM * 0.26
...
def paint_markdown(canvas: st.WordCanvas, markdown: str, *, render_mermaid: Optional[RenderMermaid] = None,
                   fetch_image: Optional[FetchImage] = None) -> None:
    """Walk `markdown` block by block and paint it onto `canvas`."""
    painter = _Painter(canvas, render_mermaid, fetch_image)
...
def render_design_docx(markdown: str, path: str, *, meta: DesignMeta,
    ...
    canvas = st.WordCanvas(title=meta.title, subject="Design document")

    # Each fact appears once. The band says what the document IS (kind, track, title,
    # the components it holds, its status); the strip says where it CAME FROM and
    # how much of the full design it is — "2 of 8 components" tells a reviewer at a
    # glance that this is a scoped document, not an unfinished one.
    canvas.title_band(
        eyebrow="DESIGN DOCUMENT",
        eyebrow_tail=meta.track,
        title=meta.title,
    ...
    canvas.footer(f"{meta.title} · Design document")
    canvas.save(path)
    return path
