...
from agents.deploying import main as deploying_app
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if not await manager.validate_auth(websocket, user_id, session_id):
                continue
...
async def process_user_message_ws(message_data: dict, websocket: WebSocket):
    ...
    try:
        ...
        if shared.prev_session_id != session_id:
            ...
            # deploying_app.invoke(state_cleanup, config=config_cleanup)
            shared.prev_session_id = session_id
        ...
        # responses = process_agent_stream_for_chat_display(deploying_app.stream(state, stream_mode="values", config=config))
...
async def handle_session_cleanup_ws(message_data: dict, websocket: WebSocket):
    ...
    try:
        ...
        if session_id_to_clean:
            ...
            # deploying_app.invoke(state, config=config)
...
async def chat(
    ...
    # Update state messages incorporating all uploaded files
        upload_messages = ", ".join(file_names)
        summary, output_file = deploying_app(user_message, file_names)
        print(f"DEBUG: Files to be used by agent (REST): {upload_messages}")
        # state["messages"].append(HumanMessage(content=f"please use the following files {upload_messages}"))
        #deploying_app.stream(state, stream_mode="values", config=config)
...
async def get_sessions():
    ...
    # if session_id!="":
    #     state = {"messages": [HumanMessage(content="cleanup if needed")]}
    #     cleanup_responses = print_stream(deploying_app.stream(state, stream_mode="values", config=config))
