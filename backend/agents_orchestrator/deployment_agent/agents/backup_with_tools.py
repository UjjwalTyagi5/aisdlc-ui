...
def _litellm_generate(prompt: str) -> str:
    """Synchronous LiteLLM call for deployment backup helpers — BYOK only.
    ...
    NOTE: this module is DEAD (no inbound imports anywhere) and not on any live
    run path. The platform-key client was removed; this path only works once a
    caller resolves a model and sets the contextvar before invoking these helpers.
    """
