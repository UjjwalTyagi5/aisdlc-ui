...
async def clear_memory_async(session_id: str = None):
    ...
    global memory, app, conversation_memory
...
"""
        ...
        response = await loop.run_in_executor(None, model.generate_content, ai_prompt)
...
"""
    ...
    try:
        ...
        loop = asyncio.get_event_loop()
        test_response = await loop.run_in_executor(None, model.generate_content, test_prompt)
        
        broadcast_log(manager, "Generating code evaluation feedback...", level="LOGS")
        code_response = await loop.run_in_executor(None, model.generate_content, code_prompt)
