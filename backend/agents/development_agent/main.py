...
from websocket_utils import set_websocket_context, real_time_print
from pydantic import BaseModel
from pathlib import Path
from util_old import save_python_file, save_zip_file, create_directory

# Import your development graph
from Nodes.node_old import graph
    ...
    try:
        while True:
            ...
            if message_data.get("type") == "user_message_with_files":
                print("Processing user message with files via WebSocket")
                await process_user_message_ws(message_data, websocket)
            elif message_data.get("type") == "development_message":
                print("Processing development message via WebSocket")
                await process_development_message_ws(message_data, websocket)
...
async def process_development_message_ws(message_data: dict, websocket: WebSocket):
    ...
    try:
        ...
        await manager.send_agent_response("Development Agent", "Processing your development request...", session_id)
            ...
            for file_data in files_data:
                ...
                try:
                    ...
                    if file_name.endswith(".py"):
                        real_time_print("Uploading Python file via WebSocket")
                        saved_files = save_python_file(mock_file, target_dir_py)
                        real_time_print("Python File Uploaded Successfully")
                    elif file_name.endswith(".zip"):
                        real_time_print("Uploading ZIP file via WebSocket")
                        saved_files = save_zip_file(mock_file, target_dir_zip)
                        real_time_print("ZIP File Uploaded Successfully")
        ...
        # Send final response
        await manager.send_agent_response("Development Agent", output_message, session_id)
        ...
        await manager.broadcast({
            ...
            "activity": {
                ...
                "type": "complete",
                "session_id": session_id,
                "message": f"Development task completed: '{user_message[:50]}...'",
                "time": "Just now"
            }
        })
        ...
        await manager.send_agent_response("Development Agent", f"Development processing error: {str(e)}", session_id)
...
async def process_user_message_ws(message_data: dict, websocket: WebSocket):
    ...
    try:
        ...
        if shared.prev_session_id != session_id:
            ...
            print(f"DEBUG: New session ({session_id}) detected. Previous session was ({shared.prev_session_id})")
            shared.prev_session_id = session_id
            await manager.send_session_update(session_id, "ready", "New session started")
        ...
        print(f"DEBUG: Initiating processing for user message: '{user_message[:100]}...'")
        
        # Process with development graph (using same pattern as development endpoint)
        response = await graph.ainvoke(state, config=config)
        final_response = response["messages"][-1].content

        # Send agent's final response to the UI (chat tab)
        await manager.send_agent_response("Processing Agent", final_response, session_id)
...
async def development_chat(
    ...
    set_websocket_context(manager, session_id)
    print(f"DEBUG: Development REST request received for session {session_id}, message: {messages[:100]}...")
