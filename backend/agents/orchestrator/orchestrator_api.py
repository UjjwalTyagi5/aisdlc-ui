...
from config.env import AGENTIC_BASE_URL, AGENTIC_INTERNAL_BASE_URL, AGENTIC_WS_URL, ANTHROPIC_API_KEY, ANTHROPIC_MODEL
...
async def call_ingestion_agent(
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=300.00) as client:
...
async def call_design_agent(enriched_input: Dict[str, Any], session_id: str, user_id: str, files_meta: List[Dict]) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=600.00) as client:
            data = _agent_form_data(enriched_input, session_id, user_id)
            response = await client.post(url, data=data, files=files)
            response.raise_for_status()
...
async def call_development_agent(enriched_input: Dict[str, Any], session_id: str, user_id: str, files_meta: List[Dict]) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=1800.00) as client:
            data = _agent_form_data(enriched_input, session_id, user_id)
            response = await client.post(url, data=data, files=files)
            response.raise_for_status()
            json_response = response.json()
...
async def orchestrate_agents(state: OrchestratorState) -> OrchestratorState:
    ...
    if combined_outputs:
        ...
        final_output = (
            ...
            "development lifecycle â€” from requirements ingestion through design, development, "
    ...
    try:
        while True:
            ...
            if message_data.get("type") == "user_message_with_files":
                await process_orchestrator_message_ws(message_data, websocket, session_id, user_id)
...
async def process_orchestrator_message_ws(message_data: dict, websocket: WebSocket, session_id: str, user_id: str):
    try:
        ...
        completion_message = f"Orchestration completed! Processed through {final_state.get('last_stage', 'unknown')} stage."
        await manager.send_agent_response("Orchestrator", completion_message, session_id)
        
    except Exception as e:
        error_message = f"Orchestration failed: {str(e)}"
        await manager.send_agent_response("Orchestrator", error_message, session_id)
