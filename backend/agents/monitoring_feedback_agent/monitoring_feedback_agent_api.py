...
monitoring_feedback_router = APIRouter()
...
@monitoring_feedback_router.websocket("/test-ws")
async def test_websocket(websocket: WebSocket):
    set_agent_folder("monitoring_feedback_agent")
...
@monitoring_feedback_router.websocket("/ws")
...
@monitoring_feedback_router.post("/chat/")
async def chat(
    session_id: str = Form(...),
    ...
    print(f"[LogAgent] REST Request received for User: {user_id}, Session: {session_id}")
    
    # --- UPDATED: Session and History Management ---
    if shared_state.prev_session_id != session_id:
        print(f"[LogAgent] New REST session detected: '{session_id}'.")
        shared_state.prev_session_id = session_id
        shared_state.chat_histories[session_id] = []
    ...
    pre_processed_input = classify_and_separate_input(user_message, has_files=bool(uploaded_files), chat_history=chat_history)
    intent = pre_processed_input["intent"]
    actual_user_question = pre_processed_input["question"]
    pasted_log_data = pre_processed_input["log_data"]

    print(f"[LogAgent] LLM classified intent as: '{intent}'")

    if intent == "follow-up":
        last_analysis_context = next((turn['content']['analysis_detail'][0] for turn in reversed(chat_history) if turn['role'] == 'assistant' and turn['content'].get('analysis_detail')), None)
        if not last_analysis_context:
            assistant_turn_content = {"responses": "I've lost the context of our last analysis. Please start over.", "analysis_detail": []}
        else:
            follow_up_answer = generate_follow_up_rest(last_analysis_context, chat_history, actual_user_question)
        ...
        response_msg = "I'm sorry, I couldn't understand your request. Please rephrase or specify if you want me to analyze logs."
        assistant_turn_content = {"responses": response_msg, "analysis_detail": []}

    elif intent == "analyze":
        print(f"[LogAgent] Starting new analysis for session: '{session_id}'. Clearing history.")
        shared_state.chat_histories[session_id] = []
        user_turn = {"role": "user", "content": user_message} # Re-set user turn for new history
        ...
        if uploaded_files:
            ...
            return {"conversation_id": session_id, "responses": "You asked me to analyze logs, but I couldn't find any. Please paste logs or upload a file.", "analysis_detail": []}

        bot_response_message = final_results[0].get('issue_description', 'Analysis complete.') if final_results and len(final_results) == 1 else (f"I analyzed the logs and found {len(final_results)} distinct issues." if final_results else "I couldn't find any actionable issues.")
        assistant_turn_content = {"responses": bot_response_message, "analysis_detail": final_results}

    # --- Save history and prepare final response ---
    shared_state.chat_histories[session_id].append(user_turn)
    shared_state.chat_histories[session_id].append({"role": "assistant", "content": assistant_turn_content})
    ...
    print(f"[LogAgent] Request processed successfully.")
    return response_data

 
@monitoring_feedback_router.get("/download/{filename}")
...
@monitoring_feedback_router.get("/sessions")
