...
// Adding someone to a unit is that unit's admin's act — same boundary as the
// role PATCH next door, and for the same reason: the directory is org-wide, so
// the write is where the scope has to be enforced.
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await getSession();
  if (!session) return Response.json({ code: "unauthenticated" }, { status: 401 });
  const { id } = await params;
  if (!canManageBusinessUnit(resolveSessionScope(session), id)) {
