...
import { listOpenPrs, prepareReview } from "@/lib/api/code-review";
...
export function ReviewTargetDialog({
  ...
  React.useEffect(() => {
    if (mode === "branch" && branchesQ.data && branchesQ.data.length > 0 && !base) {
      ...
      <DialogContent className="flex max-h-[85vh] max-w-lg flex-col">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto pr-1">
                ...
                {providers.map((sp) => (
                  <ModeButton
                    ...
                    onClick={() => {
                      ...
                      setProvider(sp.id);
                      setProject(null); setRepo(null); setSource(null); setBase(null); setPrId(null);
                    }}
                    icon={GitBranch}
                  >
                    {sp.label}
          ...
          {project && repo && mode === "branch" && (
            <>
              <Step label="Source branch (the change)">
                <CascadeList
              sourceLabel={sourceLabel}
                  q={branchesQ}
                  value={source}
              ...
              <Step label="Base branch (compare against)">
                <CascadeList
              sourceLabel={sourceLabel}
                  q={branchesQ}
                  value={base}
                  onChange={setBase}
                  getKey={(b) => `base-${b.name}`}
                  getValue={(b) => b.name}
                  getLabel={(b) => b.name}
          ...
          {project && repo && mode === "pr" && (
            <Step label="Open pull request">
              ...
              sourceLabel={sourceLabel}
                ...
                badge={(p) => `${p.source_branch} → ${p.target_branch}`}
